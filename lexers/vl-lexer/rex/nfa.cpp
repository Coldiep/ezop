...
void Nfa::TableRow::Print() const {
  std::cout << "{" << state_ << ";";
  for (CharMap::const_iterator cm_it = chars_map_.begin(); cm_it != chars_map_.end(); ++cm_it) {
...
void Nfa::AddTransition(unsigned state_from, char symbol, unsigned state_to) {
  if (state_from != ZERO_STATE) {
    StateTable::iterator table_it = transitions_.find(state_fom);
    if (table_it == transitions_.end()) {
        std::pair<StateTable::iterator, bool> res = transitions_.insert(state_from);
        table_it = res.first;
    }
    table_it->char_map_[symbol].insert(state_to);
  }
}
...
Nfa::StateSet Nfa::Move(unsigned state, char symbol) const {
  ...
  if (table_it != transitions_.end()) {
    ...
    CharMap::const_iterator states_it = table_it->char_map_.find(symbol);
    if (states_it != table_it->char_map_.end()) {
...
Nfa::SymbolSet Nfa::GetSymSet() const {
  ...
  for (StateTable::const_iterator table_it = transitions_.begin(); table_it != transitions_.end(); ++table_it) {
    ...
    CharMap::const_iterator state_it = table_it->char_map_.begin(),
      state_end_it = (*table_it).char_map_.end();
    for (CharMap::const_iterator state_it = table_it->char_map_.begin(); state_it != table_it->char_map_.end(); ++state_it) {
      if (state_it->first) {
...
void Nfa::CloneFromOffset(Nfa& new_nfa, unsigned offset) const {
  ...
  for (StateTable::const_iterator it = transitions_.begin(); it != transitions_.end(); ++it) {
    unsigned new_state_number = it->state_ + offset;
    new_nfa.AddState(new_state_number);

    const CharMap& map_from = it->char_map_;
    for (CharMap::const_iterator map_it = map_from.begin(); map_it != map_from.end(); ++map_it) {
      ...
      for (StateSet::iterator set_it = sset.begin(); set_it != set.end(); ++set_it) {
