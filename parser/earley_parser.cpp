...
void private_::state::init( earley_parser* _parser, grammar* _grammar, int _state_number, token _token )
{
  ...
  items_.resize( grammar_->get_num_of_terminals() + grammar_->get_num_of_nonterminals() + 1 ) ;
  items_with_empty_rules_.resize( grammar_->get_num_of_nonterminals() );
}
...
void private_::item::print( grammar* _grammar, std::ostream& out )
{
  ...
  out << "[ " << _grammar->get_symbol_name( _grammar->get_lhs_of_rule( rule_num_ ) ) << " --> ";
  for( int rule_pos = 0; _grammar->get_rhs_of_rule( rule_num_, rule_pos ) != -1; ++ rule_pos )
  {
    ...
    out << _grammar->get_symbol_name( _grammar->get_rhs_of_rule( rule_num_, rule_pos ) );
    out << " ";
  }
...
{
  int symbol_index = grammar_->get_rhs_of_rule( _rule_num, _rhs_pos );
  ...
  if( grammar_->get_lhs_of_rule( _item->rule_num_ ) == grammar_->get_start_symbol() && _item->origin_ == 0 )
  {
    is_completed_ = true;
  }
  ...
  {
    int empty_rule_sym_index = grammar_->get_lhs_of_rule( _rule_num ) - grammar_->get_num_of_terminals();
  ...
  // nonhandled ones.
  else if( symbol_index != -1 && grammar_->is_nonterminal( symbol_index ) )
  {
    int nonterm = symbol_index - grammar_->get_num_of_terminals();
...
inline void earley_parser::completer( item* _item )
{
  ...
  state::item_list& or_item_list = origin_state.items_[ grammar_->get_lhs_of_rule( _item->rule_num_ ) + 1 ];

  for( item* cur = or_item_list.elems_.get_first(); cur ; cur = or_item_list.elems_.get_next() )
  {
    int comp_item_index = grammar_->get_rhs_of_rule( cur->rule_num_, cur->rhs_pos_ + 1 ) + 1;
...
{
  int sym_after_dot = grammar_->get_rhs_of_rule( _item->rule_num_, _item->rhs_pos_ );
  state& cur_state = *states_[states_.size() - 1];
  
  if( ! cur_state.items_[ sym_after_dot + 1 ].handled_by_predictor_ )
  {
    private_::rule_list_t& rules_list = grammar_->get_sym_rules( sym_after_dot - grammar_->get_num_of_terminals() );
...
inline bool earley_parser::scanner()
{
  ...
  int cur_term = grammar_->get_symbol_by_id( next_sym_id );
...
inline bool   earley_parser::error_scanner()
{
  ...
  for( int i = 1; i <= grammar_->get_num_of_terminals(); ++ i )
  {
...
inline void earley_parser::closure()
{
  while( ! nonhandled_items_.empty() )
  {
    ...
    int sym_index = grammar_->get_rhs_of_rule( _item->rule_num_, _item->rhs_pos_ );

    if( sym_index == -1 )
    {
      completer( _item );
    }
    else if( grammar_->is_nonterminal( sym_index ) )
    {
      predictor( _item );
    }
  }
...
inline bool earley_parser::init_first_set()
{
  ...
  private_::rule_list_t& rules_list = grammar_->get_sym_rules( grammar_->get_start_symbol() - grammar_->get_num_of_terminals() );
  ...
  new_state.items_[ grammar_->get_start_symbol() + 1 ].handled_by_predictor_ = true;
...
inline void earley_parser::fill_rhs_stack( item* _item, rhs_stack_t& _stack )
{
  for( item* cur_item = _item; cur_item->rhs_pos_ > 0; cur_item = cur_item->lptr_ )
  {
    ...
    int symbol_index = grammar_->get_rhs_of_rule( cur_item->rule_num_, cur_item->rhs_pos_-1 );
    if( grammar_->is_nonterminal( symbol_index ) )
    {
  ...
  for( ; ! item_rhs_stack.empty(); item_rhs_stack.pop() )
  {
    ...
    switch( cur_stack_element.type_ )
    {
      ...
      case rhs_stack_element::eItem:
      {
        ...
        int symbol = grammar_->get_lhs_of_rule( cur_item->rule_num_ );
...
void earley_parser::build_parse_trees( state& _state )
{
  ...
  for( ; cur_item ; cur_item = completed_items_list.elems_.get_next() )
  {
    ...
    if( grammar_->get_start_symbol() == grammar_->get_lhs_of_rule( cur_item->rule_num_ ) && cur_item->origin_ == 0 )
    {
      
      parse_tree_t* parse_tree = new parse_tree_t();
      parse_tree_node_t* tree_head = parse_tree->add_head( grammar_->get_start_symbol() );
...
void earley_parser::print_tree( parse_tree_node_t* _node, int _level, std::ostream& _out )
{
  ...
  _out << grammar_->get_symbol_name( _node->element_ ) << std::endl;
